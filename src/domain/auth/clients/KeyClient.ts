...
export class KeyClientImpl implements KeyClient {
  ...
  getExtendPublicKey = async () => {
    ...
    return Clutch.extendedPublicKey(privateKey, extendedKeyPath(getNetworkConfig(NETWORK.ETH).bip44));
  };
  ...
  restoreServer = async () => {
    ...
    const wallet = this.ethersWalletClient.createWalletWithEntropy(privateKey, 0);
