...
const ethTransactionRepository: ContractRepository = new EthersContractImpl();

const bscTransactionRepository: ContractRepository = new EthersContractImpl();

it('constructs correctly', () => {
  expect(ethTransactionRepository).toBeDefined();
  expect(bscTransactionRepository).toBeDefined();
});

it('get eth balance', async () => {
  const balance = await ethTransactionRepository.getBalance({
...
it('get mvl balance', async () => {
  const balance = await ethTransactionRepository.getContractBalance({
    contractAddress: MVL_GOERLI.contractAddress,
    abi: JSON.stringify(testNetMvlAbi),
    address: TestData.ethAddressPKey,
    rpcUrl: TEST_ETH_RPC_URL,
  });
  expect(balance).toBe('0.0');
});
...
it('get bsc balance', async () => {
  const balance = await bscTransactionRepository.getBalance({
    selectedWalletPrivateKey: TestData.selectedWalletPrivateKey,
...
it('get bMvl balance', async () => {
  const balance = await bscTransactionRepository.getContractBalance({
    contractAddress: B_MVL_STAGE.contractAddress,
    abi: JSON.stringify(testNetbMvlAbi),
    address: TestData.bscAddressPKey,
    rpcUrl: TEST_BSC_RPC_URL,
  });
  expect(balance).toBe('0.0');
});
